...
@Tag(name = "中间件用户管理")
@RestController
@RequestMapping("/kaSecurityUser")
public class KaSecurityController {
    ...
    @PostMapping("/login")
    public BaseResponse<KaSecurityLoginUserVO> doLogin(@RequestBody KaSecurityUserLoginRequest doLoginRequest, HttpServletRequest request, HttpServletResponse response) {
...
//        String uuidDecrypt = AesUtils.decrypt((String) passWord,(String) request.getSession().getAttribute("uuid"));
        ...
        //正常登录
        KaSecurityLoginUserVO kaSecurityLoginUserVO = kaSecurityUserService.doLogin(kaSecurityUserName, kaSecurityUserPassWord, request,response);
        return ResultUtils.success(kaSecurityLoginUserVO);
    }
    ...
    public BaseResponse<KaSecurityUserVO> getVOByUserName(@PathVariable("kaSecurityUserName") String kaSecurityUserName, HttpServletRequest request) {
        ...
        return ResultUtils.success(kaSecurityUserService.getKaSecurityUserVO(kaSecurityUser));
    }
